...
public:
    ...
    ~analyseLexicale();
    string getPathfile();
    void printListReactions(){ for (auto r : listReactions) cout << r->enzyme->name << endl; };
    void printEdges(){ for (auto e : edges){
        cout << "from: ";
        for (auto i : e->fromIndices) cout << i << " ";
        cout << "to: ";
        for (auto i : e->toIndices) cout << i << " ";
        cout << endl;
    }};
    ...
    void printTrace(nodeFromTo trace){
        cout << "from: ";
        for (auto i : trace.from) cout << i << " ";
        cout << "to: ";
        for (auto i : trace.to) cout << i << " ";
        cout << endl;};
    string findPaths(int entre, int sortie, int distance);
